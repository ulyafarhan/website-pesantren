...
<nav class="flex text-sm text-gray-500 font-medium mb-8 overflow-x-auto whitespace-nowrap pb-2 scrollbar-hide" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        <li class="inline-flex items-center">
            <a href="{{ route('home') }}" class="inline-flex items-center gap-1.5 hover:text-emerald-600 transition text-gray-600 hover:bg-emerald-50 px-2 py-1 rounded-md">
        ...
        @foreach($items as $label => $url)
            <li>
                <div class="flex items-center">
                    ...
                    @if($url)
                        <a href="{{ $url }}" class="ml-1 md:ml-2 hover:text-emerald-600 transition text-gray-600 hover:bg-emerald-50 px-2 py-1 rounded-md">{{ $label }}</a>
                    @else
