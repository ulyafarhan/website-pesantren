...
<head>
    ...
    <meta name="description" content="{{ $settings->site_description ?? 'Sistem Informasi Pesantren' }}">
    <title>{{ isset($title) ? $title . ' - ' : '' }}{{ $settings->site_name ?? 'Pesantren Kita' }}</title>
...
<body class="bg-gray-50 text-gray-800 antialiased bg-topography flex flex-col min-h-screen">
    ...
    <x-navigations.navbar :settings="$settings ?? null" />

    <main class="flex-grow {{ request()->routeIs('home') ? '' : 'pt-24 lg:pt-32' }}">
        {{ $slot }}
    </main>

    <x-navigations.footer :settings="$settings ?? null" />
